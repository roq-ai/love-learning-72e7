...
import { getLessons, deleteLessonById } from 'apiSdk/lessons';
import { LessonInterface } from 'interfaces/lesson';

type ColumnType = ColumnDef<LessonInterface, unknown>;
...
export function LessonListPage(props: LessonListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getLessons({
        relations: ['content', 'student.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<LessonInterface>>(
    () => `/lessons?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: LessonInterface) => {
    if (hasAccess('lesson', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/lessons/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/lessons/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('lesson', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/lessons/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="lesson" />
        </Flex>

        {hasAccess('lesson', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/lessons/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
