...
import { getStories } from 'apiSdk/stories';
...
import { StudentListPage as ShowCaseList } from 'pages/students';

function DashboardPage() {
  const { session } = useSession();

  const storyStat = useSWR<PaginatedInterface<StoryInterface>>(
    () => `/stories`,
    () => getStories({ limit: 1, offset: 0 }),
  );

  const lessonStat = useSWR<PaginatedInterface<LessonInterface>>(
    () => `/lessons`,
    () => getLessons({ limit: 1, offset: 0 }),
  );

  const studentStat = useSWR<PaginatedInterface<StudentInterface>>(
    () => `/students`,
    () => getStudents({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Stories',
      result: storyStat?.data?.totalCount,
      isLoading: storyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Lessons',
      result: lessonStat?.data?.totalCount,
      isLoading: lessonStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Students',
      result: studentStat?.data?.totalCount,
      isLoading: studentStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`students`} passHref legacyBehavior>
                  <Button
