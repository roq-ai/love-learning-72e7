...
import { getStories, deleteStoryById } from 'apiSdk/stories';
import { StoryInterface } from 'interfaces/story';

type ColumnType = ColumnDef<StoryInterface, unknown>;
...
export function StoryListPage(props: StoryListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getStories({
        ...
        order: params.order,
        searchTermKeys: ['title.contains', 'body.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<StoryInterface>>(
    () => `/stories?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: StoryInterface) => {
    if (hasAccess('story', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/stories/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/stories/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('story', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/stories/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('story', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="story" />
        </Flex>

        {hasAccess('story', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/stories/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
